...
~~~
YEAR 함수에 입력된 인자의 개수가 맞지 않아 발생한 것입니다.  
YEAR 함수는 괄호 안에 날짜 데이터 1개만 받아야 하는데, 기존 쿼리에서는 YEAR(PUBLISHED_DATE, 2021)처럼 두 개의 값을 넣어 문법 오류가 났습니다.  
이를 해결하려면 YEAR(PUBLISHED_DATE) = 2021과 같이 날짜 컬럼에서 연도만 먼저 추출한 뒤, 그 결과값이 2021과 같은지 '=' 를 통해 확인하는 방식으로 수정해야 합니다.
~~~
